...
if 'HWW2l2nu' in args.analysis : analysis_dir = "Hpluscharm"
...
for i in input_map[args.analysis].keys():print(input_map[args.analysis][i])
...
for var in var_map.keys():
    ...
    for out in output.keys():
        ## Scale XS
        if out=="signal": output[out][var] = scale_xs(output[out][var], lumis * scales, output[out]["sumw"])
        elif out == "data":
            output[out][var] = output[out][var].group(
        "dataset", hist.Cat("plotgroup", "plotgroup"), merge_map["data"])
        else: 
            output[out][var] = scale_xs(output[out][var], lumis, output[out]["sumw"])
            output[out][var] = output[out][var].group(
    ...
    for region in args.region.split(",")[1:]:
        ...
        for chs in args.channel.split(",")[1:]:
            ...
            hep.cms.label("Work in progress", data=True, lumi=round(float(lumis/1000),1), year=year, ax=ax)
